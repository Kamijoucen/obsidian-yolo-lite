...
export const zh: TranslationKeys = {
  ...
  settings: {
    ...
    agent: {
      ...
      focusSyncTitle: '焦点同步',
      focusSyncDesc:
        '启用后，AI 可以感知你正在阅读的笔记或网页的位置。完整网页正文通过 fs_read 的 browser:// 路径读取。',
    ...
    providers: {
      ...
      deleteConfirmImpact: '这会同时删除 {chatCount} 个聊天模型。',
    ...
    models: {
      ...
      modelId: '调用ID',
      modelIdDesc:
        '用于实际 API 调用的模型标识（例如：gpt-4o-mini、deepseek-chat）',
  ...
  configTransfer: {
    ...
    errors: {
      ...
      errorSettingsVersionMismatch:
        '配置文件版本 {fileVersion} 与当前设置版本 {currentVersion} 不一致。',
      ...
      errorVaultMissingVersion: '目标笔记库的配置数据缺少必需的 version 字段。',
      errorVaultVersionMismatch:
        '目标笔记库设置版本 {vaultVersion} 与当前设置版本 {currentVersion} 不一致。',
      errorVaultEmpty: '目标笔记库的配置数据为空',
      errorApplyVersionMismatch:
        '导入数据版本 {importVersion} 与当前设置版本 {currentVersion} 不一致。',
