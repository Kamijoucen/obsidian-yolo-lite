...
如果工作区里有参考资料（你可以用 fs_list 查看有哪些文件），先用 fs_list 看看有哪些，再用 fs_read 读相关内容，基于实际内容生成大纲。在每章的 contract 里写明参考了哪个文件的哪几行（如"参考 rust-book.md 第 120-180 行"）。
...
如果章节契约里注明了参考文件（如"参考 rust-book.md 第 120-180 行"），用 fs_read 去读对应内容，确保正文有据可依。
